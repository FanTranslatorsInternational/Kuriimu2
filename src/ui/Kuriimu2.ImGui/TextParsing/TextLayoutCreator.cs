...
{
    internal class TextLayoutCreator
    {
        private readonly IReadOnlyList<CharacterInfo> _characters;

        protected LayoutOptions Options { get; }

        public TextLayoutCreator(IReadOnlyList<CharacterInfo> characters, LayoutOptions options)
        {
        ...
        protected virtual Rectangle GetCharacterBoundingBox(CharacterData character, Point characterLocation, out bool isVisible)
        {
            ...
            switch (character)
            {
                case FontCharacterData fontCharacter:
                    CharacterInfo? glyph = _characters.FirstOrDefault(x => x.CodePoint == fontCharacter.Character);
                    if (glyph == null)
                        break;
        ...
        private int GetLineHeight()
        {
            ...
            return _characters.Max(c => c.GlyphPosition.Y + c.BoundingBox.Height);
        }
    }
}
