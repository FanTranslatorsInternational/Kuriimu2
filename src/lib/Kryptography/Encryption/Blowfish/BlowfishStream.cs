...
namespace Kryptography.Encryption.Blowfish
{
    public class BlowfishStream : Stream
    {
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            {
                var d = (uint)((cipherKey[j % cipherKey.Length] << 24) | (cipherKey[(j + 1) % cipherKey.Length] << 16) |
                         (cipherKey[(j + 2) % cipherKey.Length] << 8) | cipherKey[(j + 3) % cipherKey.Length]);
                _pi[i] ^= d;
            }
        ...
        /// <param name="block">64 bit buffer to receive the merged block</param>
        protected virtual void MergeBlock(uint left, uint right, byte[] block)
        {
            // Write big endian uint to blocks
            var block1 = BitConverter.GetBytes(left);
            var block2 = BitConverter.GetBytes(right);
            Array.Reverse(block1);
            Array.Reverse(block2);
