...
namespace Kryptography.Encryption.Sony
{
    public class KirkEngine
    {
        ...
        private static readonly byte[] Fuse = new byte[0x10];   // Currently unknown

        private static readonly Dictionary<int, byte[]> Keys = new()
        {
        ...
        public static void Decrypt(byte[] inBuf, int offset, int count, byte[] outBuf, int outOffset, int keyIndex)
        {
            ...
            CopyingBlocks(inBuf, offset, count, outBuf, outOffset, Aes.CreateDecryptor());
        }
        ...
        public static void Encrypt(byte[] inBuf, int offset, int count, byte[] outBuf, int outOffset, int keyIndex)
        {
            ...
            CopyingBlocks(inBuf, offset, count, outBuf, outOffset, Aes.CreateEncryptor());
        }
        ...
        private static void CopyingBlocks(byte[] inBuf, int offset, int count, byte[] outBuf, int outOffset, ICryptoTransform transform)
        {
            ...
            {
                var size = Math.Min(BufferSize, count);
        ...
        private static void SetupAesContext(int keyIndex)
        {
            ...
            Aes.Key = keyIndex == 0x100 ? Fuse : GetKey(keyIndex) ?? throw new InvalidOperationException($"No key for index {keyIndex}.");
            Aes.IV = new byte[0x10];
        }

        private static byte[]? GetKey(int keyIndex)
        {
            return Keys.GetValueOrDefault(keyIndex);
        }
    }
}