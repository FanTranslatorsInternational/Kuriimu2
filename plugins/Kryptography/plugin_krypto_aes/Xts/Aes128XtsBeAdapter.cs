...
namespace plugin_krypto_aes.Xts
{
    ...
    [MenuStripExtension("AES", "128", "XTS", "BE")]
    public class Aes128XtsBeAdapter : ICipherAdapter
    {
        public event EventHandler<RequestDataEventArgs> RequestData;

        public string Name => "Aes128 XTS BE";
        ...
        private Task<bool> DoCipher(Stream input, Stream output, IProgress<ProgressReport> progress, bool decrypt)
        {
            ...
            var key = OnRequestKey("AES128 XTS Key", 32, requestId, out var error);
            if (key == null)
            ...
            var sectorSize = OnRequestNumber("AES128 XTS SectorSize", 512, requestId, out error);
