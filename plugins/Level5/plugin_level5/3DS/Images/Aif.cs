...
namespace plugin_level5._3DS.Images
{
    class Aif
    {
        ...
        public ImageData Load(Stream input)
        {
            ...
            var imageInfo = new ImageData(imageData, format, new Size(width, height));
        ...
        public void Save(Stream output, ImageData imageInfo)
        {
            ...
            imgSection.Data[0x10] = (byte)imageInfo.Format;
            ...
            BinaryPrimitives.WriteInt32LittleEndian(imgSection.Data[0x2C..0x30], imageInfo.Data.Length);

            var buffSection = _mainSections.First(x => x.Header.magic == " FMA").GetSection("ffub");
            BinaryPrimitives.WriteInt32LittleEndian(buffSection.Data[..0x04], imageInfo.Data.Length);

            var addrSection = _mainSections.First(x => x.Header.magic == " FMA").GetSection("rdda");
            BinaryPrimitives.WriteInt32LittleEndian(addrSection.Data[0x10..0x14], imageInfo.Data.Length);

            // Update image data
            _mainSections.First(x => x.Header.magic == " FMA").Data = imageInfo.Data;
