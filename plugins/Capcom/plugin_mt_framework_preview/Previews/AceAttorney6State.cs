...
{
    class AceAttorney6State : GenericGmdState, ITextPreviewState
    {
        private readonly IPluginFileManager _pluginManager;

        public AceAttorney6State(IPluginFileManager pluginFileManager)
        {
        ...
        private static List<IList<CharacterData>> GetPageCharacters(IList<CharacterData> characters)
        {
            ...
            {
                if (character is GmdControlCodeCharacterData { Code: "p" } or GmdControlCodeCharacterData { Code: "pa" })
                {
                    result.Add([]);
                    hasText = false;
