...
namespace plugin_mt_framework.Fonts
{
    class Gfdv2
    {
        ...
        public async Task<List<CharacterInfo>> Load(Stream input, IFileSystem fileSystem, UPath basePath, IDialogManager dialogManager)
        {
            ...
            GfdEntryv2[] entries = ReadEntries(br, _header.CharCount);
            ...
            foreach (GfdEntryv2 entry in entries)
            {
                ...
                result.Add(new CharacterInfo
                {
                    ...
                    BoundingBox = new Size(entry.CharWidth, entry.CharHeight),
                    Glyph = glyph
                });
            }
        ...
        {
            // Create entries
            IList<GfdEntryv2> entries = CreateEntries(characterInfos, out IList<Image<Rgba32>> images);
            ...
            _header.MaxCharacterWidth = 0;
            _header.MaxCharacterHeight = 0;
        ...
        private IList<GfdEntryv2> CreateEntries(IList<CharacterInfo> characterInfos, out IList<Image<Rgba32>> images)
        {
            ...
            // Create entries
            var entries = new List<GfdEntryv2>(characterInfos.Count);
            ...
            for (var i = 0; i < glyphImages.Count; i++)
            {
                ...
                foreach (PackedGlyphData glyph in glyphImages[i].Glyphs)
                {
                    ...
                    entries[^1].CharWidth = (short)characterLookup[glyph.Element.Character].BoundingBox.Width;
                    entries[^1].CharHeight = (short)characterLookup[glyph.Element.Character].BoundingBox.Height;
            ...
            foreach (CharacterInfo character in characterInfos.Where(c => c.Glyph is null))
            {
                ...
                entries[^1].CharWidth = (short)character.BoundingBox.Width;
                entries[^1].CharHeight = (short)character.BoundingBox.Height;
                entries[^1].ImageId = 0;
            }
        ...
        private GfdHeaderv2 ReadHeader(BinaryReaderX reader)
        {
            return new GfdHeaderv2
            {
                ...
                MaxCharacterWidth = reader.ReadSingle(),
                MaxCharacterHeight = reader.ReadSingle(),
                BaseLine = reader.ReadSingle(),
                DescentLine = reader.ReadSingle()
            };
        }

        private GfdEntryv2[] ReadEntries(BinaryReaderX reader, int count)
        {
            var result = new GfdEntryv2[count];
        ...
        private GfdEntryv2 ReadEntry(BinaryReaderX reader)
        {
            return new GfdEntryv2
            {
                codePoint = reader.ReadUInt32(),
                tmp1 = reader.ReadUInt32(),
                tmp2 = reader.ReadUInt32(),
                tmp3 = reader.ReadUInt32(),
        ...
        private void WriteHeader(GfdHeaderv2 header, BinaryWriterX writer)
        {
            ...
            writer.Write(header.MaxCharacterWidth);
            writer.Write(header.MaxCharacterHeight);
            writer.Write(header.BaseLine);
            writer.Write(header.DescentLine);
        }

        private void WriteEntries(IList<GfdEntryv2> entries, BinaryWriterX writer)
        {
            foreach (GfdEntryv2 entry in entries)
                WriteEntry(entry, writer);
        }

        private void WriteEntry(GfdEntryv2 entry, BinaryWriterX writer)
        {
            writer.Write(entry.codePoint);
            writer.Write(entry.tmp1);
            writer.Write(entry.tmp2);
            writer.Write(entry.tmp3);
            writer.Write(entry.posX);
            writer.Write(entry.posY);
            writer.Write(entry.endMark);
        }
